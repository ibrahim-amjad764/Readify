...
export function UrlInput({
  ...
  return (
    <div className="w-full">
      {/* Input wrapper */}
      <div
        className={`relative flex items-center gap-0 rounded-xl border transition-all duration-200 ${
          isFocused
      ...
      >
        ...
        <div className="flex items-center pl-4 pr-2 shrink-0">
          <Github
            className={`w-5 h-5 transition-colors duration-200 ${
              isFocused ? "text-[#F5A623]" : "text-[#555]"
            }`}
            aria-hidden="true"
          />
        </div>
        ...
        <input
          ...
          spellCheck={false}
          autoComplete="off"
          className={`flex-1 bg-transparent py-3.5 text-[15px] font-mono text-[#e0e0e0] placeholder-[#3a3a3a] focus:outline-none disabled:opacity-50 min-w-0 ${
        ...
        <button
          ...
          className={`btn-amber flex items-center gap-2 shrink-0 px-5 m-1.5 font-semibold transition-all disabled:opacity-40 ${
            compact ? "text-sm py-2" : "py-2.5"
          }`}
        >
          {isLoading ? (
            <>
